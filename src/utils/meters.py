...
class Meter(object):
    ...
    def accuracies(self) :
        ...
        # accs = num / (den + 1e-6)
        # acc_mean = accs[present].mean().item() * 100
        # accs = (accs.data.cpu().numpy() * 100).tolist()

        # acc_dict = edict({'iou': round(acc_mean, 2)})
        # for i in range(len(accs)) :
        #     name = self.label_to_name[i]
        #     acc_ = accs[i]
        #     acc_dict[name] = round(acc_, 2)
