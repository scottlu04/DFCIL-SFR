...
def get_clf_checkpoint_path(
        log_dir, epoch=None, 
        save_best_only=None, save_last_only=None, ) :

    if save_best_only or save_last_only : 
        return osp.join(get_checkpoint_dir(log_dir), 'clf_checkpoint_last.pth.tar')
    assert epoch is not None, f"epoch is required to save the checkpoint."
    return osp.join(get_checkpoint_dir(log_dir), 'clf_checkpoint_' + str(epoch) + '.pth.tar')
...
def get_best_clf_model_path(log_dir) :
    return osp.join(get_checkpoint_dir(log_dir), 'clf_model_best.pth.tar') 

def save_clf_checkpoint(log_dir, states, epoch, is_best=None,
                    save_best_only=None, save_last_only=None) :

    checkpoint_path = get_clf_checkpoint_path(log_dir, epoch, 
                            save_best_only, save_last_only)
    if save_last_only :
        if osp.isfile(checkpoint_path) :
            os.remove(checkpoint_path)
    ...
    if is_best :
        best_model_path = get_best_clf_model_path(log_dir)
        shutil.copyfile(checkpoint_path, best_model_path)
