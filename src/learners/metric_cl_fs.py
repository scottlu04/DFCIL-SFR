...
class Metric_CL_FS(Base):

    def __init__(self, cfg, cfg_data, args, is_train, is_distributed, n_gpus):
        super(Metric_CL_FS, self).__init__(cfg, cfg_data, args, is_train, is_distributed, n_gpus)
        self.inversion_replay = False
        self.new_class_aug = False
    ...
    def train(self, n_trial):
        ...
        for current_t_index in range(self.cfg.increm.max_task):
            ...
            # print(self.val_dataset[0]['pts'].shape)
            ...
            if current_t_index == 0 and self.cfg.increm.load_pretrained_task0:
                ...
                #pretrained_checkpoint_path ='/home/luzhenyu/DFCIL/guided_MI/output2/hgr_shrec_2017/Metric-CL-infonce/trial_3/task_0/checkpoints/checkpoint_100.pth.tar'
                ...
                pretrained_checkpoint_path = osp.join('/home/luzhenyu/DFCIL/guided_MI/models', self.args.dataset,'dg_sta_cl', f"trial_{n_trial+1}", 'checkpoints', 'model_best.pth.tar')
                ...
                if self.gen_inverted_samples:
                    ...
                    #                                self.cfg.increm.learner.inversion_batch_size, self.cfg.batch_size*8, 
                    #                                log_dir_task, self.args.log_dir)
                    
                    self.generate_inverted_samples(self.previous_teacher, self.cfg.increm.learner.n_samples_per_class, 
                                                   self.cfg.increm.learner.inversion_batch_size, self.cfg.few_shot_size*8, 
                                                   log_dir_task, self.args.log_dir)
            ...
            # self.get_feature_w_syn(0,current_t_index)
            ...
            if self.cfg.few_shot_feat_aug:
                self.new_class_aug = True
        ...
        while bi <= n_batches :
            ...
            for lname in self.criteria :
                ...
                if lname == 'mse' :
                    ...
                    lval = lfunc(feature_cl,target_com,new_idx,old_idx)
                    # lval2 =  self.Loss_try(feature_cl,target_com,new_idx,old_idx)
                    # print(lval)
                    # print(lval2)
    ...
    def train_epoch_classify(self, tbar, epoch, train_logger, current_t_index) :
        ...
        while bi <= n_batches :
            ...
            if self.new_class_aug:
                pts_aug, target_aug, target_aug_hat = self.aug_new_sample()
                pts_aug = pts_aug.cuda(self.args.gpu)
                target_aug = target_aug.cuda(self.args.gpu)
                target_aug_hat = target_aug_hat.cuda(self.args.gpu)
                feature_com, target_com = self.combine_data(((feature_com, target_com),(pts_aug, target_aug)))
    ...
    def get_feature2(self, vbar) :
        ...
        while bi <= n_batches :
            ...
            # output = output.view(bs * n_views, *pts.shape[2:]);
            # feature = feature.view(bs, n_views, *feature.shape[1:]);
            # output = output[:, 0, :]
    ...
    def get_feature(self, vbar) :
        ...
        while bi <= n_batches :
            ...
            if self.new_class_aug:
                ...
                target_aug = target_aug.cuda(self.args.gpu)
                target_aug_hat = target_aug_hat.cuda(self.args.gpu)
                feature_com, target_com = self.combine_data(((feature, target),(pts_aug, target_aug)))
            else:
                feature_com, target_com = feature, target
            output = self.classifier(feature_com.to(torch.float32))
            output = output[:, :self.valid_out_dim]
    ...
    @torch.no_grad()
    def feature_aug(self) :
        # def aug_feature_filter(target_features, targets):
        self.model.eval()
        n_batches = len(self.train_loader)
        iter_loader = iter(self.train_loader)
        bi = 1
        ...
        aug_class_data = [feat_list, target_list]
        
        #get mean and cov
        class_size = self.cfg.increm.learner.n_samples_per_class
        pre_sample_class_size = class_size * 5
        d_maha_threshold = self.cfg.d_maha_threshold
        iter_step = 2
        for i in range(iter_step):
            pca_param = 6
            proto_mean, proto_var,reduced_mean,reduced_var,feat_dict, reduced_feat_dict, directions_dict,u_dict, svd_learner = \
                get_prototype(aug_class_data, class_size, pca_param, no_filter = 1)
            ...
            target_features, targets = sampling(aug_info_dict,class_size,pre_sample_class_size, sampling_method)
            
            fpath_full = osp.join(self.args.log_dir,'info_dict.pkl')
            info_dict_full  = utils.load_pickle(fpath_full)
            print(info_dict_full['proto_mean'].keys())
            # target_features, targets = aug_feature_filter(target_features, targets)
            ...
            # filtered_size = pre_sample_class_size
            
            rej_dict = {}
            for new_id in target_features:
                while (1):
                    rej_dict[new_id] = set()
                    # count = 0
                    for idx, feat in enumerate(target_features[new_id]):
                        # count = 0
                        for old_id in info_dict_full['proto_mean']:        
                            # count +=1
                            mean = info_dict_full['proto_mean'][old_id][0]
                            cov = info_dict_full ['proto_var'][old_id]
                        ...
                        for new_other_id in aug_info_dict['proto_mean']:        
                            # count +=1
                            # print('compare')
                            # print(new_other_id)
                            # print(new_id)
                            if new_id == new_other_id:
                                continue 
                            mean = aug_info_dict['proto_mean'][new_other_id][0]
                            cov = aug_info_dict ['proto_var'][new_other_id]
                            cov =np.diag(np.diag(cov))
                            val = mahalanobis_distance(feat,mean,cov)
                            if not np.isnan(val):
                                if val < d_maha_threshold:
            ...
            if i == iter_step-1: # output the feature
                for id in proto_mean:
                    target_features[id] =np.delete(target_features[id], list(rej_dict[id]), axis=0)  #target_features[id][accpet_dict[id],:
                    print(target_features[id].shape)
                    idxs = np.random.RandomState(1).choice(target_features[id].shape[0], class_size, replace=False)
                    target_features[id] = target_features[id][idxs]
                    targets[id] = targets[id][:class_size]
            else:
                feat_list = []
                target_list = []
                for id in proto_mean:
                    target_features[id] =np.delete(target_features[id], list(rej_dict[id]), axis=0)  #target_features[id][accpet_dict[id],:
                    feat_list.append(target_features[id])
                    # print(target_features[id].shape)
                    target_list.append(targets[id][:len(target_features[id])])
                feat_list = np.concatenate(feat_list, axis = 0)#.cpu().numpy()
                target_list = np.concatenate(target_list, axis = 0)
                aug_class_data = [feat_list, target_list]
        ...
        for id in targets:
            targets_list.append(targets[id])
            features_list.append(target_features[id])
        targets_list = np.concatenate(targets_list)  
        features_list =  np.concatenate(features_list) 

        utils.stdio.save_pickle(osp.join(self.args.log_dir, 'aug_features.pkl'), [target_features,targets])

        self.aug_new_dataset = InvertedDataset(features_list, targets_list, targets_list)
        dataloader_batch_size = self.cfg.batch_size
        self.aug_new_dataloader = DataLoader(self.aug_new_dataset, batch_size=dataloader_batch_size, shuffle=True, pin_memory=True)
        self.aug_new_iterator = iter(self.aug_new_dataloader)

    def aug_new_sample(self):
        # Return a batch of inverted samples
            try:
                inputs, targets, outputs = next(self.aug_new_iterator)
