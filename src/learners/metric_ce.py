...
class Metric_CE(Base):

    def __init__(self, cfg, cfg_data, args, is_train, is_distributed, n_gpus):
        super(Metric_CE, self).__init__(cfg, cfg_data, args, is_train, is_distributed, n_gpus)
    ...
    def train(self, n_trial):
        ...
        for current_t_index in range(self.cfg.increm.max_task):
            ...
            if current_t_index == 0 and self.cfg.increm.load_pretrained_task0:
                ...
                pretrained_checkpoint_path = osp.join('/home/luzhenyu/DFCIL/guided_MI/models', self.args.dataset,'stgcn', f"trial_{n_trial+1}", 'checkpoints', 'model_best.pth.tar')
            ...
            # elif self.cfg.increm.learner.type == 'cts':
            self.get_feature(0)
            # self.get_val_feature(0,current_t_index)
            self.get_feature_w_syn(0,current_t_index,True, -1)
            self.sample_shape = (-1, self.cfg.seq_len, self.cfg.model.n_joints, self.cfg.in_channels)
