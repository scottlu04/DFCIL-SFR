...
class Metric_CL(Base):

    def __init__(self, cfg, cfg_data, args, is_train, is_distributed, n_gpus):
        super(Metric_CL, self).__init__(cfg, cfg_data, args, is_train, is_distributed, n_gpus)
    ...
    def train(self, n_trial):
        ...
        for current_t_index in range(self.cfg.increm.max_task):
            ...
            if current_t_index == 0 and self.cfg.increm.load_pretrained_task0:
                ...
                pretrained_checkpoint_path = osp.join('./models', self.args.dataset,'dg_sta_cl', f"trial_{n_trial+1}", 'checkpoints', 'model_best.pth.tar')
            ...
            classifer_type = 'linear' 
            if classifer_type =='linear':
                # if current_t_index == 0:
