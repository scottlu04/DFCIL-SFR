...
class Base(object):
    ...
    def train(self, n_trial):
        ...
        for current_t_index in range(self.cfg.increm.max_task):
            ...
            #         'Dataset')('test', self.args.split_type, self.cfg_data, self.cfg.transforms['test'], 
            #         self.add_classes, self.known_classes, rm_global_scale=self.cfg.rm_global_scale, drop_seed=n_trial)
            ...
            if current_t_index == 0 and self.cfg.increm.load_pretrained_task0:
                ...
                pretrained_checkpoint_path = osp.join('/home/luzhenyu/DFCIL/guided_MI/models', self.args.dataset, self.cfg.model.name, f"trial_{n_trial+1}", 'checkpoints', 'model_best.pth.tar')
                ...
                for epoch in range(start_epoch, total_epochs_task + 1) :
                    ...
                    measures, per_class_accuracy = self.validate_epoch(vbar, epoch, val_logger) 
            ...
            if self.cfg.increm.learner.type == 'deep_inversion' or self.cfg.increm.learner.type == 'abd':
            ...
            elif self.cfg.increm.learner.type == 'wts' or self.cfg.increm.learner.type =='wts_ib':
                self.get_feature(0)
                self.get_val_feature(0,current_t_index)
                self.get_feature_w_syn(0,current_t_index)
                self.sample_shape = (-1, self.cfg.seq_len, self.cfg.model.n_joints, self.cfg.in_channels)
                self.previous_teacher = Teacher_v3(solver=copy.deepcopy(self.model), 
                                                    sample_shape = self.sample_shape, 
                                                    iters = self.power_iters, 
                                                    deep_inv_params = self.deep_inv_params, 
                                                    class_idx = np.arange(self.valid_out_dim), 
                ...
                self.previous_teacher.generate_target_feature(self.args.log_dir,50)
                self.inversion_replay = True
                self.gen_inverted_samples = True
            else:
    ...
    @torch.no_grad()
    def get_feature_w_syn(self, vbar,current_t_index) :
        ...
        while bi <= n_batches :
            ...
            if self.cfg.model.name == 'dg_sta_var':
                output,feature, mu, std = self.model(pts_com)
            else:
    ...
    def validate_epoch(self, vbar, epoch, val_logger) :
        ...
        return return_values, per_class_accuracy
