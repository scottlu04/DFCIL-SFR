...
class SpatioTemporalAttention(nn.Module):
    ...
    def __init__(self, 
        ...
        self.att_layer2 = MultiHeadedAttention(
                        n_heads, 
                        d_head, 
                        d_in, 
                        dom_type,
        ...
        self.att_layer3 = MultiHeadedAttention(
                        n_heads, 
                        d_head, 
                        d_in, 
                        dom_type,
    ...
    def forward(self, 
        ...
        # print(x.size())
        #x = self.att_layer2(x) #pass attention model
        #x = self.att_layer3(x) #pass attention model
        x = self.linear(x) 
        return x
    ...
    def init_parameters( self ) :
       # model_list = [ self.att_layer, self.linear]
        model_list = [ self.att_layer, self.linear,self.att_layer2,self.att_layer3]
...
class Model(nn.Module):
    def __init__(self, 
        ...
        self.z_prior = torch.empty(n_classes, d_feat)
        self.fc_mu = nn.Linear(d_feat, d_feat)
        self.fc_logvar = nn.Linear(d_feat, d_feat)
        self.decoder = nn.Linear(d_feat, n_classes)
        gain = 3
        self.gain = gain
        ...
        nn.init.normal_(self.decoder.weight, 0, math.sqrt(2. / n_classes))
    ...
    def forward(self, 
        ...
        feature = torch.mean(x, dim=1) # bs x d_out
        
        z_mu = self.fc_mu(feature)
        z_logvar = self.fc_logvar(feature)
        z = self.reparameterize(z_mu, z_logvar)

        x = self.decoder(z) # bs x n_classes
        # import sys 
        # sys.exit()
        return x, z, z_mu, z_logvar
