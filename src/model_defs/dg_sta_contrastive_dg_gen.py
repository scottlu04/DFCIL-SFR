...
class MultiHeadedAttention(nn.Module) :
    ...
    def forward_cross_attention(self, x, key):
        """
        Args :
        x : Tensor, shape [bs x n x n_h * d_h]
        Returns :
        ...
        q = self.q_map(x); # bs x n_h x n x d_h
        k = self.k_map(key); # bs x n_h x n x d_h
        v = self.v_map(key); # bs x n_h x n x d_h
...
class Encoder(nn.Module):
    ...
    def forward_spatial_temp_feature(self, 
        x: Tensor, # bs x seq_len x n_joints x in_channels
    ) -> Tensor :
        ...
        x = self.temporal_att(x); # bs x seq_len * n_joints x d_out
        feature_temp = x
        feature_spatial = torch.mean(feature_spatial, dim=1);
        feature_temp = torch.mean(feature_temp, dim=1);
        x = torch.mean(x, dim=1); # bs x d_out
        return x,  feature_spatial, feature_temp
...
class Decoder(nn.Module):
    ...
    def forward(self, 
        z_spatial: Tensor, # bs x seq_len x n_joints x in_channels
        z_temp: Tensor,

    ) -> Tensor :

        assert not torch.any(torch.isnan(z_spatial));
        assert not torch.any(torch.isnan(z_temp));
        #print(z_spatial.size())
        bs = z_spatial.size(0)
        timequeries = torch.zeros(bs, self.n_pts,  self.d_feat, device=z_spatial.device)
        print(timequeries.size())
        print(z_spatial.size())
        x = self.spatial_att.forward_decode(timequeries, z_spatial);
        #print(x.size())
        x = self.temporal_att.forward_decode(x, z_temp); # bs x seq_len * n_joints x d_out
       # x = torch.mean(x, dim=1); # bs x d_out
        return x; 
...
class Model(nn.Module):
    def __init__(self, 
      ...
      #  self.decoder = 
        self.spatial_mu_layer = nn.Linear(d_feat, self.d_model)
        self.spatial_sigma_layer = nn.Linear(d_feat, self.d_model)
        self.temporal_mu_layer = nn.Linear(d_feat, self.d_model)
        self.temporal_sigma_layer = nn.Linear(d_feat, self.d_model)
        ...
        self.finallayer = nn.Linear(d_feat, self.in_channels)
    ...
    def decode(self, 
        ...
        bs = z_spatial.size(0)
        z_spatial = z_spatial + self.actionBiases[y]
        z_spatial =z_spatial[None]
        z_spatial = z_spatial.view(bs,1,-1) 
        z_temp = z_temp + self.actionBiases[y]
        z_temp =z_temp[None] 
        z_temp = z_temp.view(bs,1,-1) 
        output = self.decoder(z_spatial, z_temp)
        #print(output.size())
        output = self.finallayer(output).reshape(bs, self.seq_len, self.n_joints, self.in_channels)
        print(output.size())
    ...
    def forward_contrastive(self, 
        ...
        z, z_spatial, z_temp = self.encoder.forward_spatial_temp_feature(x); # bs x d_out
        #print(x.size())
        bs = x.size(0)
        #print(bs)
        mu_spatial =  self.spatial_mu_layer(z_spatial)
        var_spatial =  self.spatial_sigma_layer(z_spatial)
        mu_temp = self.temporal_mu_layer(z_temp)
        var_temp = self.temporal_sigma_layer(z_temp)
        
        z_spatial = self.reparameterize(mu_spatial,var_spatial)
        z_temp = self.reparameterize(mu_temp,var_temp)
        
        output = self.decode(z_spatial,z_temp,y)
        return output 
