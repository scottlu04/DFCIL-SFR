...
class Model(nn.Module):
    def __init__(self, 
        ...
        self.muQuery = nn.Parameter(torch.randn(n_classes, self.d_model))
        self.sigmaQuery = nn.Parameter(torch.randn(n_classes, self.d_model))


        self.skelEmbedding = nn.Linear(150, self.d_model)
        ...
        self.finallayer = nn.Linear(self.d_model, 150)
    ...
    def decode(self, 
        ...
        #print(output.size())
        output = self.finallayer(output).reshape(self.seq_len, bs, 150)
    ...
    def forward_contrastive(self, 
        ...
        x = x.view(self.seq_len, x.size(0), self.n_joints* self.in_channels)#.reshape(nframes, bs, njoints*nfeats)
    ...
    def forward(self, 
        ...
        #print(x.size())
        #print(self.in_channels)
        x = x.view(self.seq_len, x.size(0), -1)#.reshape(nframes, bs, njoints*nfeats)
        x = self.skelEmbedding(x)
        xseq = torch.cat((self.muQuery[y][None], self.sigmaQuery[y][None], x), axis=0)
        xseq = self.sequence_pos_encoder(xseq)
